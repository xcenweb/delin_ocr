                ...
                <v-card class="mb-4">
                    ...
                    <v-card-text>
                        <v-text-field v-model="ocrForm.relative_path" label="relative_path (相对路径)" density="compact"
                            class="mb-2" />
                        <v-text-field v-model="ocrForm.tags" label="tags (标签)" density="compact" class="mb-2" />
                        ...
                        <v-list v-if="ocrRecords.length > 0" density="compact">
                            <v-list-item v-for="ocr in ocrRecords.slice(0, 3)" :key="ocr.id">
                                ...
                                <p class="text-caption">
                                    tags: {{ ocr.tags || 'other' }} | text: {{ ocr.text }}<br>
                                    update: {{ ocr.update_time || '' }}<br>
                                    create: {{ ocr.create_time || '' }}
                                </p>
...
import { ocrRecordsDB, type OCRRecord } from '@/utils/dbService'
...
const addOCR = async () => {
    ...
    try {
        ...
        ocrForm.value = { relative_path: '', tags: '', text: '', block: '' }
        await loadOCRs()
    } catch (error) {
